...
function confidenceLabel(value: number | undefined): 'High' | 'Medium' | 'Low' {
  if (value === undefined) return 'High' // parser-direct references default High
...
function nodeRelationship(node: ChainNode, edges: ChainGraph['edges']): RelationshipMeta {
  ...
  switch (node.status) {
    case 'alias_resolved':
      return {
        explanation: 'This reference resolves to the linked canonical authority.',
        type: 'alias',
    ...
    case 'ambiguous':
      return {
        explanation: 'This reference matched multiple authorities; the closest match is shown.',
        type: 'references',
        confidence: 'Medium',
        method: 'parser',
      }
...
function ResolveCard({ data, onSelectSection }: { data: ResolvedProvision; onSelectSection?: (id: string) => void }) {
  return (
    <div className="resolve-card">
      ...
      {data.legal_relationships.length > 0 && (
        <div className="preview-row">
    ...
    <div className={`node-row${isMissing ? ' node-row-missing' : ''}`} style={{ paddingLeft: node.depth * 20 }}>
      ...
      {isMissing && (
        <div className="node-missing-note">
          Reference recognized — full text not yet loaded in corpus.
          {(() => {
...
export default function LawNavigatorPage() {
  ...
  return (
    <main className="page">
      <header className="site-header">
        ...
        <p className="page-eyebrow">Expand References</p>
